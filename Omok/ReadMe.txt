...
Omok.h
    ...
    헤더(Resource.h 포함)가 들어 있고 COmokApp 응용 프로그램
    클래스를 선언합니다.

Omok.cpp
    응용 프로그램 클래스 COmokApp이(가) 들어 있는 기본 응용 프로그램
    소스 파일입니다.
...
OmokDoc.h, OmokDoc.cpp - 문서
    이 파일에는 COmokDoc 클래스가 들어 있습니다. 이 파일을 편집하여
    특수한 문서 데이터를 추가하고 COmokDoc::Serialize를 통해 파일
    저장 및 로드를 구현합니다.

OmokView.h, OmokView.cpp - 문서 뷰
    이 파일에는 COmokView 클래스가 들어 있습니다.
    COmokView 개체는 COmokDoc 개체를 보는 데 사용됩니다.
...
StdAfx.h, StdAfx.cpp
    이 파일은 미리 컴파일된 헤더 파일(PCH)인 Omok.pch와
